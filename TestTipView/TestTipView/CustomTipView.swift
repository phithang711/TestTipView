...
public protocol CustomTipViewDelegate: AnyObject {
...
public extension CustomTipView {
    
    /**
     Presents an CustomTipView pointing to a particular UIView instance within the specified superview
     
     - parameter animated:    Pass true to animate the presentation.
     - parameter view:        The UIView instance which the CustomTipView will be pointing to.
     - parameter superview:   A view which is part of the UIView instances superview hierarchy. Ignore this parameter in order to display the CustomTipView within the main window.
     - parameter text:        The text to be displayed.
     - parameter preferences: The preferences which will configure the CustomTipView.
    ...
    class func show(animated: Bool = true,
                    ...
                    text: String,
                    preferences: Preferences = CustomTipView.globalPreferences,
                    delegate: CustomTipViewDelegate? = nil) {
        
        let ev = CustomTipView(text: text, preferences: preferences, delegate: delegate)
    ...
    class func show(animated: Bool = true,
                    ...
                    preferences: Preferences = CustomTipView.globalPreferences,
                    delegate: CustomTipViewDelegate? = nil) {
        let ev = CustomTipView(contentView: contentView, preferences: preferences, delegate: delegate)
        ev.show(animated: animated, forView: view, withinSuperview: superview)
    }
    
    /**
     Presents an CustomTipView pointing to a particular UIView instance within the specified superview containing attributed text.
     - parameter animated:       Pass true to animate the presentation.
     - parameter view:           The UIView instance which the CustomTipView will be pointing to.
     - parameter superview:      A view which is part of the UIView instances superview hierarchy. Ignore this parameter in order to display the CustomTipView within the main window.
    ...
    class func show(animated: Bool = true,
                    ...
                    preferences: Preferences = CustomTipView.globalPreferences,
                    delegate: CustomTipViewDelegate? = nil) {
        let ev = CustomTipView(text: attributedText, preferences: preferences, delegate: delegate)
        ev.show(animated: animated, forView: view, withinSuperview: superview)
    }
    ...
    /**
     Dismisses the CustomTipView
     
     - parameter completion: Completion block to be executed after the CustomTipView is dismissed.
     */
    func dismiss(withCompletion completion: (() -> Void)? = nil) {
        ...
        UIView.animate(withDuration: preferences.animating.dismissDuration, delay: 0, usingSpringWithDamping: damping, initialSpringVelocity: velocity, options: [.curveEaseInOut], animations: {
            ...
            completion?()
            self.delegate?.customTipViewDidDismiss(self)
...
// MARK: - CustomTipView class implementation -
open class CustomTipView: UIView {
    ...
    public struct Preferences {
        ...
        public struct Positioning {
            ...
            public var maxWidth             = CGFloat(UIScreen.main.bounds.size.width * 320 / 390)
        }
        ...
        public var hasShadow: Bool {
            drawing.shadowOpacity > 0 && drawing.shadowColor != UIColor.clear
        }
    ...
    fileprivate weak var delegate: CustomTipViewDelegate?
    ...
    fileprivate lazy var contentSize: CGSize = {
        ...
        case .text(let text):
            ...
            var textSize = text.boundingRect(with: CGSize(width: self.preferences.positioning.maxWidth, height: CGFloat.greatestFiniteMagnitude), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: attributes as [NSAttributedString.Key: Any], context: nil).size
    ...
    public convenience init (text: String, preferences: Preferences = CustomTipView.globalPreferences, delegate: CustomTipViewDelegate? = nil) {
    ...
    public convenience init (contentView: UIView, preferences: Preferences = CustomTipView.globalPreferences, delegate: CustomTipViewDelegate? = nil) {
        self.init(content: .view(contentView), preferences: preferences, delegate: delegate)
    }
    
    public convenience init (text: NSAttributedString, preferences: Preferences = CustomTipView.globalPreferences, delegate: CustomTipViewDelegate? = nil) {
        self.init(content: .attributedText(text), preferences: preferences, delegate: delegate)
    }
    
    public init (content: Content, preferences: Preferences = CustomTipView.globalPreferences, delegate: CustomTipViewDelegate? = nil) {
    ...
    @objc func handleRotation() {
        guard let sview = superview, presentingView != nil else {
            return
        }
    ...
    fileprivate func arrange(withinSuperview superview: UIView) {
        ...
        if !isFrameValid(frame, forRefViewFrame: refViewFrame, withinSuperviewFrame: superviewFrame) {
            for value in ArrowPosition.allValues where value != position {
                ...
                if isFrameValid(newFrame, forRefViewFrame: refViewFrame, withinSuperviewFrame: superviewFrame) {
                    ...
                    if position != .any {
                        print("[CustomTipView - Info] The arrow position you chose <\(position)> could not be applied. Instead, position <\(value)> has been applied! Please specify position <\(ArrowPosition.any)> if you want CustomTipView to choose a position for you.")
                    }
        ...
        case .bottom, .top, .any:
            ...
            arrowTip = CGPoint(x: arrowTipXOrigin,
                               y: position == .bottom
    ...
    @objc func handleTap() {
        self.delegate?.customTipViewDidTap(self)
        guard preferences.animating.dismissOnTap else {
            return
        }
        dismiss()
    }
...
#else
        let attributes = [NSAttributedStringKey.font: preferences.drawing.font,
    ...
    private func getContentRect(from bubbleFrame: CGRect) -> CGRect {
        CGRect(x: bubbleFrame.origin.x + preferences.positioning.contentInsets.left,
